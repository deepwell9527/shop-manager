...
import {get, isArray, isFunction, isUndefined, set} from 'lodash'
...
export const coverSourceArrayToObj = (arr) => {
    ...
    const checkDataType = (type, value) => {
        ...
        if (type === 'bool') {
            return ((value !== 'false' && value !== 0) && (value === 'true' || value))
        }
    }
...
export const handlerDictProps = (item, tmpArr) => {
    ...
    if (allowCoverComponent.includes(item.formType)) {
        data = tmpArr.map(dicItem => {
            let label = get(dicItem, (item.dict.props && item.dict.props.label) || labelName, '')
            let valueDesc = get(dicItem, item.dict.props && item.dict.props.valueDesc, '')
            label = label + (valueDesc ? '(' + valueDesc + ')' : '')
