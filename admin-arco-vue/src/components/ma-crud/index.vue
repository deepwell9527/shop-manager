...
<template>
  <a-layout-content class="flex flex-col lg:h-full relative w-full">
    <div ref="crudHeaderRef" class="_crud-header flex flex-col mb-2">
      <a-tabs
      ...
      >
        ...
        <a-tab-pane v-for="item in options.tabs.data" :key="item.value" :title="item.label">
          <template #title>
    ...
    <div class="_crud-content">
      <div ref="crudOperationRef" class="operation-tools lg:flex justify-between mb-3">
        <a-space class="lg:flex block">
          ...
          <slot name="tableButtons">
            <a-button
                ...
                class="w-full lg:w-auto mt-2 lg:mt-0" type="primary"
                @click="addAction"
            >
              <template #icon>
                <icon-plus/>
            ...
            >
              <a-button
                  ...
                  class="w-full lg:w-auto mt-2 lg:mt-0" status="danger"
                  type="primary"
              >
                <template #icon>
                  <icon-delete/>
            ...
            >
              <a-button
                  ...
                  class="w-full lg:w-auto mt-2 lg:mt-0" status="success"
                  type="primary"
              >
                <template #icon>
                  <icon-undo/>
          ...
          <slot :selectedIds="selecteds" name="tableAfterButtons"></slot>
        </a-space>
        <a-space v-if="options.showTools" class="lg:mt-0 mt-2">
          ...
          <a-tooltip v-if="!options.hideToolToggleSearch" content="显隐搜索">
            <a-button shape="circle" @click="toggleSearch">
              <icon-search/>
            </a-button>
          </a-tooltip>
          <a-tooltip v-if="!options.hideToolPrintTable" content="打印表格">
            <a-button shape="circle" @click="printTable">
              <icon-printer/>
            </a-button>
          </a-tooltip>
          <a-tooltip v-if="!options.hideToolTableSetting" content="设置">
      ...
      <div ref="crudContentRef">
        <slot name="content" v-bind="tableData">
          ...
          >
            ...
            <template v-if="options.showExpandRow" #expand-row="record">
            ...
            <template v-if="options.customerSummary || options.showSummary"
    ...
    <div v-if="total > 0 && openPagination && !options.tablePagination" ref="crudFooterRef"
         class="_crud-footer mt-3 text-right">
      <a-pagination
          v-model:current="requestParams[config.request.page]"
          :page-size="requestParams[config.request.pageSize]" :page-size-options="options.pageSizeOption" :total="total"
          show-jumper
          show-page-size
          show-total
    ...
    <a-image-preview-group v-if="typeof imgUrl === 'object' && imgUrl !== null" v-model:visible="imgVisible"
                           :srcList="imgUrl"/>
    <a-image-preview v-else v-model:visible="imgVisible" :src="imgUrl"/>
  </a-layout-content>
</template>
...
defineExpose({
  refresh, requestData, addAction, editAction, getTableData, setSelecteds, dictTrans,
