...
<template>
  <component
      ...
      :fullscreen="options.formOption.isFull || false" :on-before-ok="submit"
  ...
  >
    ...
    <a-spin :loading="dataLoading" class="w-full" tip="加载中...">
      <ma-form ref="maFormRef" v-model="form" :columns="formColumns" :options="formOptions">
...
import {cloneDeep, get, isArray, isFunction, isUndefined} from 'lodash'
...
const formItemShow = (item) => {
  // @DeepWell
  let display1 = null
  if (currentAction.value === 'add' && item.hasOwnProperty('addDisplay')) {
    display1 = item.addDisplay !== false
  } else if ((currentAction.value === 'edit' || currentAction.value === 'see') && item.hasOwnProperty('editDisplay')) {
    display1 = item.editDisplay !== false
  }
  let display2 = item.display !== false
  if (display1 === null) {
    return display2
  } else {
    return display1
